...
class VolatilityEngine:
    ...
    def __init__(self):
        ...
        # Free tier = 800/day. Daily candle fetch = 2 credits per 5-min cycle = 576/day wasted.
        self._saturation_cache = {pair: (0.0, None) for pair in PAIRS}  # (value, hour_fetched)
    ...
    def _fetch_candles(self, pair: str) -> list:
        ...
        params = {
            'symbol':     pair,   # TwelveData forex requires slash: EUR/USD not EURUSD
            'interval':   '5min',
            'outputsize': 32,
    ...
    def _get_daily_range_saturation(self, pair: str) -> float:
        ...
        Saves ~480 credits/day on 800/day free tier.
        """
        try:
            ...
            try:
                ...
                for pair in PAIRS:
                    try:
                        ...
                        consecutive_errors = 0
                    # Stagger pair calls by 15s to stay under 8 calls/min rate limit
                    time.sleep(15)
